...
> {
  ...
  async fetchAllCurrencies() {
    try {
      ...
      this.setState(
        { ...this.state, allCurrencies: res.data.currencies },
        () => {
          ...
          <ul className="currency-list">
            {this.state.allCurrencies.map((item: Currency, i: number) => {
              return (
                <li
                  className={`item ${
                    item.symbol === this.props.activeCurrency.symbol
                      ? "selected"
                      : ""
                  }`}
                  key={i.toString()}
...
const dispatchToProps = (dispatch: Dispatch) => {
  return {
    initialCurrencySet: (currency: Currency) =>
      dispatch({
        type: REDUX_ACTIONS.ACTIVE_CURRENCY_UPDATE,
        payload: currency,
      }),
  };
};
